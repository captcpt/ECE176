...
class KNN(object):
    ...
    def predict(self, x_test: np.ndarray, k: int = None, loop_count: int = 1):
        ...
        # TODO: implement me
        #pass
        neighbors_indices = np.argsort(distance, axis=1)[:, :k_test]
        neighbors_labels = self._y_train[neighbors_indices]
        predicted_labels = np.apply_along_axis(lambda x: np.bincount(x).argmax(), axis=1, arr=neighbors_labels)
    ...
    def calc_dis_one_loop(self, x_test: np.ndarray):
        ...
        # TODO: implement me
        #pass
        distance = np.zeros((x_test.shape[0], self._x_train.shape[0]))

        distance = np.zeros((x_test.shape[0], self._x_train.shape[0]))

        for i in range(x_test.shape[0]):
            distance[i, :] = np.sqrt(np.sum((x_test[i, np.newaxis] - self._x_train) ** 2, axis=1))
