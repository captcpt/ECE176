...
class Logistic(object):
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray, weights: np.ndarray) -> np.ndarray:
        ...
        # TODO: implement me
        y_one_hot = np.zeros((N, self.n_class))
        ...
        for epoch in range(self.epochs):
            ...
            loss = -np.sum(np.log(probabilities) * y_one_hot) / N + 0.5 * self.weight_decay * np.sum(self.w ** 2)
            grad = np.dot((probabilities - y_one_hot).T, X_train) / N + self.weight_decay * self.w
